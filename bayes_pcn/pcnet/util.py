...
def kernel_log_prob(X_train: torch.Tensor, Y_train: torch.Tensor,
                    ...
                    Sigma_trtr_invs: List[torch.Tensor]) -> torch.Tensor:
    ...
    Args:
        ...
        Sigma_trtr_invs (List[torch.Tensor]): A list of training data precision matrices.
    ...
    mu_posterior, Sigma_posterior = kernel_posterior_params(
                                        ...
                                        Sigma_trtr_inv=Sigma_trtr_invs)
    log_prob = 0.
    for j in Y_test.shape[-1]:
        log_prob += dists.MultivariateNormal(mu_posterior[:, j], Sigma_posterior[:, :, j])
    return log_prob


def kernel_posterior_params(X_train: torch.Tensor, Y_train: torch.Tensor, X_test: torch.Tensor,
                            kernel: Kernel, kernel_params: Dict[str, Any],
                            Sigma_trtr_invs: List[torch.Tensor]
    ...
    Returns:
        Tuple[torch.Tensor, torch.Tensor]: Predicted label matrix of shape <n_test x y_dim>
            and its covariance tensor of shape <n_test x n_test x y_dim>.
    """
    y_dim = Y_train.shape[-1]
    mu_posteriors, Sigma_posteriors = [], []
    for j in range(y_dim):
        mu_post_j, Sigma_post_j = marginal_kernel_posterior_params(
                                      X_train=X_train, y_train=Y_train[:, j], X_test=X_test,
                                      kernel=kernel, kernel_params=kernel_params,
                                      Sigma_trtr_inv=Sigma_trtr_invs[j])
        mu_posteriors.append(mu_post_j)
        Sigma_posteriors.append(Sigma_post_j)
    mu_posterior = torch.stack(mu_posteriors, dim=-1)
    Sigma_posterior = torch.stack(Sigma_posteriors, dim=-1)
    return mu_posterior, Sigma_posterior


def marginal_kernel_posterior_params(X_train: torch.Tensor, y_train: torch.Tensor,
                                     X_test: torch.Tensor, kernel: Kernel,
                                     kernel_params: Dict[str, Any], Sigma_trtr_inv: torch.Tensor
                                     ) -> Tuple[torch.Tensor, torch.Tensor]:
    """Single output GP forward operation.
    ...
    Args:
        ...
        kernel (Kernel): Kernel type (ex. Kernel.RBF).
        kernel_params (Dict[str, Any]): Kernel hyperparameter dictionary.
        Sigma_trtr_invs (torch.Tensor): Training data precision matrix of shape <n_train x n_train>.

    Returns:
        Tuple[torch.Tensor, torch.Tensor]: Predicted label vector of shape <n_test x 1>
            and its covariance matrix of shape <n_test x n_test>.
    """
    mu_train, mu_test = 0., 0.
    if kernel == Kernel.RBF:
        _, Sigma_trte, Sigma_tete = rbf_sigmas(X_train=X_train, X_test=X_test, **kernel_params)
    elif kernel == Kernel.RELU:
        _, Sigma_trte, Sigma_tete = relu_sigmas(X_train=X_train, X_test=X_test, **kernel_params)
    else:
        raise NotImplementedError
...
def rbf_sigmas(X_train: torch.Tensor, X_test: torch.Tensor, lengthscale: float, Sigma_obs: float
    ...
    X_trtr_dot = X_train.matmul(X_train)
    ...
    X_trte_dot = X_train.matmul(X_test)
    X_tete_dot = X_test.matmul(X_test)
...
def relu_sigmas(X_train: torch.Tensor, X_test: torch.Tensor, Sigma_obs: float
                ) -> List[torch.Tensor, torch.Tensor, torch.Tensor]:
    n_train, n_test = X_train.shape[0], X_test.shape[0]
    Phi_train = torch.nn.functional.relu(X_train)
    Phi_test = torch.nn.functional.relu(X_test)
    Sigma_trtr = Phi_train.matmul(Phi_train.T)
    Sigma_trtr = Sigma_trtr + Sigma_obs * torch.eye(n_train).to(Sigma_trtr.device)
    if X_test is None:
        return Sigma_trtr, None, None

    Sigma_trte = Phi_train.matmul(Phi_test)
    Sigma_tete = Phi_test.matmul(Phi_test)
    Sigma_tete = Sigma_tete + Sigma_obs * torch.eye(n_test).to(Sigma_tete.device)
    return Sigma_trtr, Sigma_trte, Sigma_tete
